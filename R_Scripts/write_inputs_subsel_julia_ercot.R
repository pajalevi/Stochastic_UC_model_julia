...
makeoutputs(firstdays = first, days = alldays, input_folder = gams_fol,output_folder = output_fol)
...
################################
makeoutputs = function(days, firstdays, id, input_folder, output_folder){
  ...
  ###### solar input ####
  # load full dataset
  solar = read.table(file = paste0(input_folder,"solar_input_8760.txt"), sep=",", row.names = 1, header = T)
  # ugh this converts colnames
  colnames(solar) = c("SOLAR-1","SOLAR-2","SOLAR-3","SOLAR-4","SOLAR-5","SOLAR-6","SOLAR-7","SOLAR-8")
  # solar = read_csv(file = paste0(input_folder,"solar_input_8760.txt"), sep=",", row.names = 1, header = T)
  ...
  ######## wind input ######
  # load full dataset
  wind = read.table(file = paste0(input_folder,"wind_input_8760.txt"), sep=",", row.names = 1, header = T)
  ...
  dr = read.table(file = paste0(input_folder,"dr_availability_PJM_onpeak.csv"), sep=",", row.names = 1, header=T)
  colnames(dr) = c("DR-1")
