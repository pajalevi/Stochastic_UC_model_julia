  # full_universe_stoch_net_demand.jl
#=
branched from full_universe_stoch.jl on Nov 15 2018

solves the full universe version of a stochastic two stage unit-commitment model
in which slow generators are committed in thefirst stage and fast generators can
be committed in real time. Demand response functions as a slow generator with an
advance commmitment not only of startup but also to a generation schedule

In this  version, there are TWO sources of uncertainty:
(1) demand response reliability
(2) net demand uncertainty
Both are specified via three things:
1) the levels of the scenarios
2) the probaiblities of the scenarios
3) first_periods designates which periods are independent

Currently, two periods can have 100% or 0% correllation. Future iterations
...
#=
  ...
  X set up tests for runtime on sherlock (different number time periods, omegas)
